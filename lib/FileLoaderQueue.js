...
FileLoaderQueue["prototype"] = Object.create(FileLoaderQueue, {
    ...
    "length":       { "get": function()  { return this._highPriorityQueue.length + this._midPriorityQueue.length; }},
});
...
function FileLoaderQueue_add(url,           // @arg URLString
                             ...
                             readyCallback, // @arg Function - readyCallback(result:Any, url:URLString, code:HTTPStatusCodeUINT16, options:Object):void
...
function FileLoaderQueue_clear(clearCallback) { // @arg Function = null - clearCallback():void
    ...
    for (var i = 0, iz = this._highPriorityQueue.length; i < iz; ++i) {
        if (this._highPriorityQueue[i]) {
            ...
            _fireDetailError(job.errorCallback, error, job.url, 500);
        }
    }
    for (i = 0, iz = this._midPriorityQueue.length; i < iz; ++i) {
        if (this._midPriorityQueue[i]) {
            job = this._midPriorityQueue[i]; // url, retryCount, ...
            _fireDetailError(job.errorCallback, error, job.url, 500);
        }
    }
    ...
    if (length && this._stopped === false) {
        ...
        if (this._queueState.indexOf("nop") >= 0) {
            this._queueState.forEach(function(state, index) {
                ...
                FileLoader["load"](job.url, job.responseType, function(result, url, code) {
                    if (that._stopped) {
                        _fireDetailError(job.errorCallback, "cancel", url, code);
                    } else {
                        job.readyCallback(result, url, code, job.options);
                    }
                    that._queueState[index] = "nop"; // locked -> nop
                }, function(error, url, code) {
                    if (that._stopped) {
                        _fireDetailError(job.errorCallback, error, url, code);
                    }
