...
function _dump(url, xhr) {
    if (IN_NW || IN_EL) {
        ...
        var indexFile   = dir + "index.json";
        var json        = _loadIndex(indexFile);
        var itemIndex   = _itemIndexCounter++;

        var bodyFile    = dir + itemIndex + "." + URI["getExt"](url);
        var header      = xhr["getAllResponseHeaders"](); // all response header
        var body        = xhr["response"]; // arraybuffer / text / json ...

        _addIndex(json, itemIndex, url, header);
        _saveIndex(indexFile, json);

        switch (xhr["responseType"] || "text") {
        case "text":
            fs["writeFile"](bodyFile, body, "utf8");
            break;
        case "json":
            fs["writeFile"](bodyFile, JSON.stringify(body), "utf8");
            break;
        case "document":
            fs["writeFile"](bodyFile, body["documentElement"]["outerHTML"], "utf8");
            break;
        case "arraybuffer":
            fs["writeFileSync"](bodyFile, new Buffer(body));
            break;
        case "blob":
            _convertBlobToArrayBuffer(body, function(arrayBuffer) {
                fs["writeFileSync"](bodyFile, new Buffer(arrayBuffer));
            });
        }
    }

    function _loadIndex(path) { // @ret JSONObject
        var json = {};
    ...
    function _saveIndex(path, json) {
        fs["writeFileSync"](path, JSON.stringify(json, null, 2), "utf8");
    }

    function _addIndex(json, itemIndex, url, header) {
        //  json = {
        //      itemIndex: {
        //          "url": URLString,
